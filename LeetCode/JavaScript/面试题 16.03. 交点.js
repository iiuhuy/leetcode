...
var intersection = function(start1, end1, start2, end2, k = 0) {
  ...
  if (start1[0] - end1[0] === 0) {
    if (start2[0] - end2[0] === 0) {
      ...
      if (
        start1[0] === start2[0] &&
        Math.max(start1[1], end1[1]) >= Math.min(start2[1], end2[1]) &&
        Math.min(start1[1], end1[1]) <= Math.max(start2[1], end2[1])
      )
        return [
          start1[0],
          Math.max(Math.min(start2[1], end2[1]), Math.min(start1[1], end1[1]))
        ];
    } else {
      // 第二条线不平行Y(未判断线段可交)
    ...
    if (start2[1] - end2[1] === 0) {
      ...
      if (
        start1[1] === start2[1] &&
        Math.max(start1[0], end1[0]) >= Math.min(start2[0], end2[0]) &&
        Math.min(start1[0], end1[0]) >= Math.max(start2[0], end2[0])
      )
        return [
          Math.max(
            Math.min(start2[0], end2[0]),
            Math.min(start1[0], end1[0]),
            start1[1]
          )
        ];
